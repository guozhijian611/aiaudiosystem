"""
Translate2 Node 配置管理模块
负责加载和管理基于Whisper-Diarization的文本转写节点配置
"""
...
class Config:
    ...
    WHISPER_COMPUTE_TYPE = os.getenv('WHISPER_COMPUTE_TYPE', 'auto')  # 计算精度
    
    # 说话人分离配置（Whisper-Diarization增强版）
    ENABLE_DIARIZATION = os.getenv('ENABLE_DIARIZATION', 'true').lower() == 'true'
    DIARIZATION_MODEL = os.getenv('DIARIZATION_MODEL', 'pyannote/speaker-diarization-3.1')
    HF_TOKEN = os.getenv('HF_TOKEN', '')  # Hugging Face Token，必需
    ...
    VAD_MAX_SPEECH_DURATION = float(os.getenv('VAD_MAX_SPEECH_DURATION', 30.0))
    
    # TitaNet嵌入配置
    ENABLE_TITANET = os.getenv('ENABLE_TITANET', 'true').lower() == 'true'
    TITANET_MODEL = os.getenv('TITANET_MODEL', 'titanet-l')
    ...
    WHISPER_DIARIZATION_PATH = os.getenv('WHISPER_DIARIZATION_PATH', './whisper-diarization')
    ...
    def __str__(self):
        ...
        VAD启用: {self.ENABLE_VAD}
        TitaNet启用: {self.ENABLE_TITANET}
