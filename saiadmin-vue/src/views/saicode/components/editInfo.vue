    ...
    <a-spin :loading="loading" tip="加载数据中..." class="w-full">
      <a-form :model="form" ref="formRef">
        <a-tabs v-model:active-key="activeTab">
          ...
          <a-tab-pane title="字段配置" key="field_config">
            ...
            <a-table
              ...
              <template #columns>
                ...
                <a-table-column dataIndex="column_width" title="列表宽度" :width="100">
                  <template #cell="{ rowIndex }"
                    ><a-input-number v-model="form.columns[rowIndex].column_width"
                  /></template>
                </a-table-column>
                ...
                <a-table-column dataIndex="is_insert" title="表单" :width="60">
                  <template #title
                    >表单
                    <a-tooltip content="全选 / 全不选" position="bottom"
                ...
                <a-table-column dataIndex="is_edit" title="查看" :width="60">
                  <template #title
                    >查看
                    <a-tooltip content="全选 / 全不选" position="bottom"
                ...
                <a-table-column dataIndex="view_type" title="页面控件" :width="210">
          ...
          <a-tab-pane title="菜单功能" key="menu_config">
            <a-alert title="提示"> 选择不同的功能，将会生成对应功能的代码 </a-alert>
            <a-checkbox-group
              direction="vertical"
...
const open = async (id) => {
  ...
  const response = await api.readTable(id)
...
const handleChange = (_data) => {
  ...
  _data.forEach((element, index) => {
    element.list_sort = index
  })
  form.value.columns = _data
}
...
const save = async (done) => {
  ...
  if (form.value.namespace == 'saiadmin') {
    Message.error('请修改应用名称，禁止使用saiadmin')
    return false
  }
  form.value.options = formOptions.value
  const response = await api.update(form.value.id, form.value)
...
const init = () => {
  ...
  // 请求表字段
  api.getTableColumns({ table_id: record.value.id, orderBy: 'list_sort', orderType: 'asc' }).then((res) => {
    form.value.columns = []
    res.data.map((item) => {
