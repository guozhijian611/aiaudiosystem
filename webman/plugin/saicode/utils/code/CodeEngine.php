...
class CodeEngine
{
    ...
    {
        return [
            'template_path' => base_path().DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.'saicode'.DIRECTORY_SEPARATOR.'utils'.DIRECTORY_SEPARATOR.'code'.DIRECTORY_SEPARATOR.'stub',
    ...
    public function generateTemp(): void
    {
        ...
        $viewOutPath = $vuePath . $subPath . DIRECTORY_SEPARATOR . $this->value['business_name']. DIRECTORY_SEPARATOR . 'view.vue';
        $this->checkPath($viewOutPath);
        $viewContent = $this->renderContent('vue', 'view.stub');
        file_put_contents($viewOutPath, $viewContent);

        $apiOutPath = $vuePath . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . $this->value['business_name'] . '.js';
        $this->checkPath($apiOutPath);
        $apiContent = $this->renderContent('js', 'api.stub');
        file_put_contents($apiOutPath, $apiContent);
