...
class QueueConsumer:
    """队列消费者 - 处理文本转写任务（Whisper-Diarization版本）"""
    ...
    def __init__(self):
        ...
        logger.info(f"Translate2 Node 队列消费者初始化完成: {self.config.QUEUE_NAME}")
    ...
    def _init_transcriber(self):
        ...
        if self.transcriber is None:
            logger.info("正在初始化Whisper-Diarization转写器...")
            self.transcriber = WhisperDiarizationTranscriber()
            logger.info("Whisper-Diarization转写器初始化完成")
    ...
    def _download_audio(self, voice_url: str, task_id: int) -> str:
        ...
        try:
            ...
            logger.info(f"音频文件下载成功: {local_path} (大小: {self._format_size(file_size)})")
            return local_path
    ...
    def stop_consuming(self):
        ...
        try:
            ...
            logger.error(f"停止消费队列时出错: {e}")
    
    def get_status(self) -> dict:
        """获取消费者状态"""
        return {
            'queue_name': self.config.QUEUE_NAME,
            'connected': self.connection and not self.connection.is_closed if self.connection else False,
...
def main():
    """主函数"""
    logger.info("Translate2 Node 启动中...")
    logger.info("=" * 50)
    logger.info("Whisper-Diarization 转写节点 v2.0.0")
    logger.info("功能: 基于 Whisper-Diarization 的音频转文本服务")
    logger.info("特性: 增强的说话人分离、VAD、TitaNet 嵌入")
    logger.info("=" * 50)
    ...
    logger.info("Translate2 Node 已停止")
